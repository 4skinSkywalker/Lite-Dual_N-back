...
Engine.prototype.onSettingChange = function(obj, key) {
    ...
    if (key == "blocks" || key == "n") {
        ...
        onChangeAttacher(el, function() {
            ...
            this.howlerizer(sel, obj[key]["selection"][sel]);
        });
    }
}
...
Engine.prototype.howlerizer = function(dir, a) {
    ...
    a.forEach(function(el) {
        this.loadedSounds.push(new Howl({
...
Engine.prototype.markupInitializer = function() {
    ...
    this.calculateStimuli(this.blocks["value"], this.n["value"]);
    this.functionizer("#" + this.btnTrg, "this.start()", this.playSymbol);
}
...
Engine.prototype.start = function() {
    ...
    this.functionizer("#" + this.btnTrg, "this.stop()", this.stopSymbol);
}
...
Engine.prototype.playBlock = function() {
    ...
    if (++this.blockCounter < this.currBlockLen) {
        ...
        this.playing = setTimeout(this.playBlock, this.time["value"]);
        this.enable = [0, 0];
    } else {
        var date = new Date();
        ...
        var incorrectVis = this.userScore[1] + this.userScore[2],
            incorrectAud = this.userScore[4] + this.userScore[5],
            threshold = this.blocks["value"] * (1 - this.threshold["value"]),
